...
class uint(struxx):
  ...
  def get_version(self, fd):
      return self.ioctl(fd, HIDIOCGVERSION)
  def get_flags(self, fd):
      return self.ioctl(fd, HIDIOCGFLAG)
  def set_flags(self, fd):
      return self.ioctl(fd, HIDIOCSFLAG)
 
class hiddev_devinfo(struxx):
  _fields = "bustype;busnum;devnum;ifnum;vendor;product;version;num_applications"
  _format = "IIIIhhhI"
  def get(self, fd):
      return self.ioctl(fd, HIDIOCGDEVINFO)
...
def _IO(x,y):
    return FIX(IOC_NONE|(ord(x)<<8)|y)
def _IOR(x,y,t):
    return FIX(IOC_READ|((t&IOCPARM_MASK)<<16)|(ord(x)<<8)|y)
def _IOW(x,y,t):
    return FIX(IOC_WRITE|((t&IOCPARM_MASK)<<16)|(ord(x)<<8)|y)
def _IOWR(x,y,t):
    return FIX(IOC_READ|IOC_WRITE|((t&IOCPARM_MASK)<<16)|(ord(x)<<8)|y)
...
def HIDIOCGNAME(buflen):
    return _IOR('H', 0x06, buflen)
...
def HIDIOCGPHYS(buflen):
    return _IOR('H', 0x12, buflen)
...
def enum_reports(fd):
  for report_type in (HID_REPORT_TYPE_INPUT, HID_REPORT_TYPE_OUTPUT, HID_REPORT_TYPE_FEATURE):
